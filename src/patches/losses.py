...
class AdversarialLoss(SetwiseLoss):
    ...
    def process_slcwa_scores(
        ...
        self_adv_weights = negative_scores.detach().mul(self.inverse_softmax_temperature).softmax(dim=-1)
            
        if neg_weights is not None:
        # Moltiplicazione element-wise. Entrambi i tensori devono avere shape [batch_size, num_negatives]
            neg_weights = self_adv_weights * neg_weights
        else:
            neg_weights = self_adv_weights
