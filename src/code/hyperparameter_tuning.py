...
parser.add_argument("--sampler", type=str, default='schema',
                    choices=['bernoulli', 'pseudotyped', 'schema'],
...
model_configs = {
    'transe': {
        'model_kwargs_ranges': dict(
            embedding_dim=dict(type='categorical', choices=[128, 256, 512]),
            scoring_fct_norm=dict(type='categorical', choices=[1, 2]), 
        )
    },
    'rotate': {
        'model_kwargs_ranges': dict(
...
hpo_kwargs = dict(
    ...
    load_if_exists=True,
    n_trials=2,
    training_kwargs=dict(num_epochs=3, batch_size=512),
    training=train,
    validation=val,
    testing=test,
    model=MODEL,
    ...
    negative_sampler_kwargs_ranges=dict(
        num_negs_per_pos=dict(type=int, low=15, high=60, q=15)),
...
optimization_result.save_to_directory(f'hpo_results/{args.g}/{MODEL}_{SAMPLER}_hpo_results')
print('study_saved')
